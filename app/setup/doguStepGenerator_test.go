...
func Test_doguStepGenerator_GenerateSteps(t *testing.T) {
	...
	t.Run("generate dogu steps with service account dependencies", func(t *testing.T) {
		...
		assert.Equal(t, "Wait for pod with selector dogu=ldap to be ready", doguSteps[2].GetStepDescription())
		assert.Equal(t, "Installing dogu [cas]", doguSteps[3].GetStepDescription())
		assert.Equal(t, "Wait for pod with selector dogu=cas to be ready", doguSteps[4].GetStepDescription())
		assert.Equal(t, "Installing dogu [postgres]", doguSteps[5].GetStepDescription())
		assert.Equal(t, "Wait for pod with selector dogu=postgres to be ready", doguSteps[6].GetStepDescription())
		assert.Equal(t, "Wait for pod with selector dogu=postfix to be ready", doguSteps[7].GetStepDescription())
